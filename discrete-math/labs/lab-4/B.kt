...
private val PROBLEM_NAME = "chvatal"
...
private fun solve(`in`: BufferedReader, out: BufferedWriter) {
    ...
    for (k in 0..n * (n - 1)) { // forEach is not rational in this situation
        if (!matrix[queue[head]][queue[head + 1]]) {
            ...
            var middle = head + 2
            while ((middle - head != n) &&
                    (!matrix[queue[head]][queue[middle]] || !matrix[queue[head + 1]][queue[middle + 1]])) middle++
            if (middle - head == n) { middle = head + 2; while (!matrix[queue[head]][queue[middle]]) middle++ }
