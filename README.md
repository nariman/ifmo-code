...
- Для доступа к БД реализовать свой `SQLiteOpenHelper`, который в процессе работы приложения должен создаваться в единственном. экземпляре и использоваться для получения инстанса `SQLiteDatabase`.
...
- При записи данных в БД использовать явные транзакции и `SQLiteStatement` для оптимизации
