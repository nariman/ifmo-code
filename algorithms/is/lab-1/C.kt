...
private const val PROBLEM_NAME = "turtle"
...
private fun solve(`in`: Scanner, out: BufferedWriter) {
    val h = `in`.int()
    val w = `in`.int()
    val matrix = Array(h + 1) { IntArray(w + 1) { 0 } }
    val res = Array(h + 1) { IntArray(w + 1) { 0 } }

    for (x in h downTo 1) for (y in 1..w) matrix[x][y] = `in`.int()
    for (x in 1..h) for (y in 1..w) res[x][y] = Math.max(res[x - 1][y], res[x][y - 1]) + matrix[x][y]
